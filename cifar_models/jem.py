...
class DummyModel(nn.Module):
    ...
    def refined_logits(self, x, n_steps=1):
        xs = x.size()
        dup_x = x.view(xs[0], 1, xs[1], xs[2], xs[3]).repeat(1, 5, 1, 1, 1)
        dup_x = dup_x.view(xs[0] * 5, xs[1], xs[2], xs[3])
        dup_x = dup_x + torch.randn_like(dup_x) * 0.03
        refined = self.refine(dup_x, n_steps=n_steps, detach=False)
        logits = self.logits(refined)
        logits = logits.view(x.size(0), 5, logits.size(1))
    ...
    def refine(self, x, n_steps=1, detach=True):
    ...
    def logpx_delta_score(self, x, n_steps=1):
